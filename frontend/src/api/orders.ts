...
export interface OrdersApiOptions {
  ...
  baseUrl?: string
  /**
   * Owner bearer token. Omit (or pass null) for unauthenticated access --
   * both the Server (order taking) and Kitchen (order status) views use
   * this endpoint with no login, so this is normally left unset.
   */
  token?: string | null
}
